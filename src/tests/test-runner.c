...
set_xdg_runtime_dir(void)
{
	...
	assert((snprintf(xdg_runtime_dir, PATH_MAX, "%s",
...
rmdir_xdg_runtime_dir(void)
{
	const char *xrd_env = getenv("XDG_RUNTIME_DIR");
        char socket_file[PATH_MAX], socket_lock_file[PATH_MAX];
	assert(xrd_env && "No XDG_RUNTIME_DIR set");

        snprintf(socket_file, sizeof(socket_file), "%s/%s", xrd_env, "wayland-0");
        snprintf(socket_lock_file, sizeof(socket_lock_file), "%s/%s", xrd_env, "wayland-0.lock");
        if (unlink(socket_file) < 0)
          perror("Cleaning socket file");
        if (unlink(socket_lock_file) < 0)
          perror("Cleaning socket lock file");
}
...
int main(int argc, char *argv[])
{
	...
	if (is_debugger_attached()) {
		...
		leak_check_enabled = !getenv("TZSH_TEST_NO_LEAK_CHECK");
		timeouts_enabled = !getenv("TZSH_TEST_NO_TIMEOUTS");
	}
